...
export const FilterPanelComp: React.FC<FilterPanelProps> = ({
  form,
  label,
  title,
  hidden,
  ...
  const [showFilter, setShowFilter] = useState(showFilterProp || false);
  ...
  const handleSearch = () => {
    setShowFilter(false);
    form!.validateFields((errors, values) => searchFunc(values));
  };
  ...
  return (
    <div className="inline-block" hidden={hidden}>
      <Drawer
